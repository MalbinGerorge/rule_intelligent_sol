...
Cypher: MATCH (r:Rule {customer_id: $customer_id})-[:HAS_CONDITION]->(c:Condition {field: "Command"}) RETURN r.name, c.operator, c.values

Question: "Which rules depend on the building block SYSTEM-1300?"
Cypher: MATCH (r:Rule {customer_id: $customer_id})-[rel:REFERENCES]->(bb:Rule {identifier: "SYSTEM-1300", customer_id: $customer_id}) RETURN r.name, properties(rel) AS reference_info

Question: "Which rules detect MITRE technique T1078?"
Cypher: MATCH (r:Rule {customer_id: $customer_id})-[:DETECTS_TECHNIQUE]->(t:MitreTechnique {technique_id: "T1078"}) RETURN r.name
...
Cypher: MATCH (r:Rule {customer_id: $customer_id})-[rel:REFERENCES]->(bb) WHERE rel.threshold_count > 100 RETURN r.name, rel.threshold_count, bb.name
